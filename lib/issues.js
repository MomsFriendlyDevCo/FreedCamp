				...
				.then(fcId => {
					...
					if (fcId) {
						...
						return axios(this.auth.getAxiosPrototype({
							...
							.then(({data}) => {
								if (!Array.isArray(data.data?.issues)) throw new Error('Expected FCIssues to return an array but got something else');
								if (data.data.issues.length == 0) throw new Error(`Issue "${ref}" not found`);
								if (data.data.issues.length != 1) throw new Error(`Expected FCIssues to return 1 issue for ref "${ref}", got ${data.data.issues.length}`);
								return data.data.issues[0];
							})
							.then(issue => this.injectIssue(issue))
					}
