...
export default class FCIssues {
	...
	* @param {Boolean} [options.global=false] If false constrain issues to the primary project, if true act as if we are searching all projects
	...
	get(ref, options) {
		...
		return Promise.resolve()
			.then(()=> this.cache.worker({
				...
				.then(()=> debug(`Issue "${ref}" not in cache, refreshing ${settings.global ? 'GLOBAL ' : ''}cache...`))
				...
				.then(fcId => {
					...
					if (fcId) {
						...
						return axios(this.auth.getAxiosPrototype({
							...
							.then(({data}) => {
								if (data.data.issues.length != 1) throw new Error(`Expected FCIssues to return 1 issue got ${data.data.issues.length}`);
								return data.data.issues[0];
							})
							.then(issue => this.injectIssue(issue))
					} else { // Make full refresh request
						debug(`Search for ref "${ref}"`, settings.global ? 'GLOBALLY' : 'in project');
						return axios(this.auth.getAxiosPrototype({
							method: 'GET',
							url: `/issues`,
							params: {
							...
							.then(({data}) => {
								if (!Array.isArray(data.data?.issues)) {
									throw new Error('Expected FCIssues to return an array but got something else');
								} else if (data.data.issues.length == 0) {
									throw new Error(`Issue "${ref}" not found`);
								} else {
									let found = data.data.issues.find(i => i.number_prefixed == ref);
									if (!found) throw new Error(`Expected FCIssues to return 1 issue for ref "${ref}", got ${data.data.issues.length} and none match the issue ref exactly`);
									return found;
								}
							})
							.then(issue => this.injectIssue(issue))
