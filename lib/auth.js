	...
	*/
	init(options) {
		this.config = new DotEnv()
			.parse(['.env'])
			.set({
				...(options?.secret && {FREEDCAMP_SECRET: options.secret}),
				...(options?.apikey && {FREEDCAMP_APIKEY: options.apikey}),
				...(options?.project && {FREEDCAMP_PROJECT: options.project}),
				...(options?.cache && {FREEDCAMP_CACHE_METHOD: options.cache}),
			})
			.schema({
